...
/**
 * 获取目录总大小或者文件的大小.
 * @param fdir_name 目录路径，或者文件路径
 * @return 目录大小，或者文件大小
 */
int64_t AnalogFS::ftwsize(const char *fdir_name)
{
        struct stat64 file_stat;

		if(stat64(fdir_name, &file_stat) == -1)
		{
			return 0;
		}
		/*这是一个普通文件---zd---2015/7/21*/
		if(S_ISREG(file_stat.st_mode))
		{
			return file_stat.st_size;
		}

		DIR* dir = opendir(fdir_name);
        if(dir == NULL) {
                pwarning(_("[%s] [%d] opendir on \"%s\" failed: %s"),__FILE__, __LINE__, fdir_name, strerror(errno));
                return 0;
        }
