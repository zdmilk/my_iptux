...
/* 系统调用，屏幕截图工具 */
void DialogPeer::screen(DialogPeer *dlgpr)
{
    GdkPixbuf *pixbuf;
    ...
    GtkWidget *widget;
    //DialogBase *dlgpr;
    char *filename = "/tmp/screen/screen.jpg";
    char *type = "jpeg";
    char *filepath = "/tmp/screen";
    system("moses-screenshot");

    if(access(filepath,0) == -1)
    {
        system("mkdir /tmp/screen");
    }
    clipboard = gtk_clipboard_get_for_display(gdk_display_get_default(),GDK_SELECTION_CLIPBOARD);
    pixbuf = gtk_clipboard_wait_for_image(clipboard);
    succ = gdk_pixbuf_save(pixbuf,filename,type,&error,NULL);
    if(succ == FALSE)
    {
        perror("Failed to save the pixbuf");
        exit(1);
    }
...
GtkWidget *DialogPeer::CreateToolMenu()
{
        ...
        /*屏幕截图*/
		//NO_OPERATION_C
		menuitem = gtk_image_menu_item_new_with_mnemonic(_("_Jscreen"));
		gtk_menu_shell_append(GTK_MENU_SHELL(menu), menuitem);
        g_signal_connect_swapped(menuitem, "activate", G_CALLBACK(screen), this);
		gtk_widget_add_accelerator(menuitem, "activate", accel,
...
void DialogPeer::ShowInfoEnclosure(DialogPeer *dlgpr)
{
    ...
    receiving = 0;
    /* 获取文件图标 */
    pixbuf = obtain_pixbuf_from_stock(GTK_STOCK_FILE);
    dpixbuf = obtain_pixbuf_from_stock(GTK_STOCK_DIRECTORY);
    //设置界面显示
	if(DialogPeer::TransFlag == 0)    //标记文件是否处于正在传输状态---zd
	{
	...
	if(receiving > 0 && DialogPeer::TransFlag == 0)
	{
...
void DialogPeer::ReceiveFile(DialogPeer *dlgpr)
{
   ...
   // dlgpr->timerrcv = g_idle_add(GSourceFunc(UpdataEnclosureRcvUI), dlgpr);
}
/**
 * 接收文件数据.
 * @param file 文件信息
